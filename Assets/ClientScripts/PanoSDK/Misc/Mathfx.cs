...
public sealed class Mathfx
{
    ...
    //Boing
    public static float Berp(float start, float end, float value,float p1 = 0.2f,float p2 = 2.5f,float p3 = 2.2f, float p4 = 1.2f)
    {
        value = Mathf.Clamp01(value);
        value = (Mathf.Sin(value * Mathf.PI * (p1 + p2 * value * value * value)) * Mathf.Pow(1f - value, p3) + value) * (1f + (p4 * (1f - value)));
        return start + (end - start) * value;
    }

    public static Vector2 Berp(Vector2 start, Vector2 end, float value, float p1, float p2, float p3, float p4)
    {
        return new Vector2(Berp(start.x, end.x, value,p1,p2,p3,p4), Berp(start.y, end.y, value, p1, p2, p3, p4));
    }

    public static Vector3 Berp(Vector3 start, Vector3 end, float value,float p1, float p2, float p3, float p4)
    {
        return new Vector3(Berp(start.x, end.x, value, p1, p2, p3, p4), Berp(start.y, end.y, value, p1, p2, p3, p4), Berp(start.z, end.z, value, p1, p2, p3, p4));
    }
    ...
    //Bounce
    public static float Bounce(float x,float p1 = 6.28f)
    {
        return Mathf.Abs(Mathf.Sin(p1 * (x + 1f) * (x + 1f)) * (1f - x));
    }

    public static Vector2 Bounce(Vector2 vec, float p1)
    {
        return new Vector2(Bounce(vec.x,p1), Bounce(vec.y, p1));
    }

    public static Vector3 Bounce(Vector3 vec, float p1)
    {
        return new Vector3(Bounce(vec.x, p1), Bounce(vec.y, p1), Bounce(vec.z, p1));
    }
